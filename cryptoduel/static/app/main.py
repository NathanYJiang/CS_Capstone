...
import string
import numpy as np
from cryptoduel.app.quotes import db
import random
...
class InteractiveDecoder:
    ...
    def start(self):
        print(f"Solve this cryptogram by {self.author[:-1]} by providing a letter substitution.")
        
        while True:
            print(f"Original Code: {self.encrypted_message}")
            self.display_current_guess()
            guess = input("Enter a letter substitution in the format 'A=B' (or 'A=' to clear, 'enter' to submit, 'exit' to quit): ").upper()
            
            if guess == "EXIT":
                print(f"Puzzle ended. The quote was: {self.cipher.decrypt(self.encrypted_message)}")
                break
            elif guess == "ENTER":
                # Check if the user has correctly decoded the message
                if "".join(self.guessed_message).upper() == self.cipher.decrypt(self.encrypted_message):
                ...
                try:
                    if "=" in guess:
                        ...
                        if original_letter == guessed_letter:
                            print("Self-decoding is not allowed. Please choose different letters.")
                            continue

                        # Clear the letter if the user inputs an empty guessed_letter
                        if guessed_letter == "":
                            print(f"Clearing letter {original_letter} from the current guess.")
                        
                        # Update the current guess with the substitution
                        self.update_guess(original_letter, guessed_letter)
                    else:
                        print("Invalid format. Please enter in the format 'A=B' or 'A=' to clear.")
                except ValueError:
...
# Example usage
def main():
    message, author = random.choice(db)
    author = author.split(",")[0]
    cipher = Cipher()
    encrypted = cipher.encrypt(message)
    interactive_decoder = InteractiveDecoder(encrypted, cipher, author)
    interactive_decoder.start()
